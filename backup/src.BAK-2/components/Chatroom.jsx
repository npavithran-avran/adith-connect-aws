...
function Chatroom({ session }) {
  ...
  const [contactIdState, setContactIdState] = useState(currentContactId[0]);
  useEffect(() => {
    setContactIdState(currentContactId[0]);
  }, [currentContactId]);
      ...
      <ul className="chats" ref={messageEl}>
        {console.log(chatMessages)}
        {
          ...
          chatMessages.map((chat) => {
            // console.log(chat, chat.contactId, currentContactId[0]);
