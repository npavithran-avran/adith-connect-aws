...
function Chatroom({ session }) {
  ...
  const [connectedContacts] = useGlobalState("connectedContacts");
  ...
  useEffect(() => {
    var currentlyConnected = connectedContacts
      .map((contact) => (!endedContacts.includes(contact) ? contact : null))
      .filter((n) => n);

    if (currentlyConnected.includes(currentContactIdState)) {
